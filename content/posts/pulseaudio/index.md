...
preview: /content/blog/pulseaudio/images/logo.png
tags:
    - Linux
    - Sound
categories:
    - Technology
date: 2023-11-04T15:53:37.903Z
---
{{< figure src="images/pulse_audio.png" title="Diagram of PulseAudio" >}}
...
{{< figure src="components.png" title="Diagram of Components" >}}
...
![](/articles/pulseaudio-under-the-hood/diagrams/devices_and_streams.png)
...
![](/articles/pulseaudio-under-the-hood/diagrams/object_hierarchy.png)
...
![](/articles/pulseaudio-under-the-hood/diagrams/dbus.png)
...
![](/articles/pulseaudio-under-the-hood/diagrams/async_api.png)
...
![](/articles/pulseaudio-under-the-hood/diagrams/tunnels.png)
...
![](/articles/pulseaudio-under-the-hood/diagrams/rtp.png)
...
![](/articles/pulseaudio-under-the-hood/diagrams/device_drivers.png)
...
![](/articles/pulseaudio-under-the-hood/diagrams/alsa_kernel_interfaces.png)
...
![](/articles/pulseaudio-under-the-hood/diagrams/alsa_user_space_interfaces.png)
...
[![](/articles/pulseaudio-under-the-hood/diagrams/alsa_ucm.png)](/articles/pulseaudio-under-the-hood/diagrams/alsa_ucm.png)
...
[![](/articles/pulseaudio-under-the-hood/diagrams/alsa_no_ucm.png)](/articles/pulseaudio-under-the-hood/diagrams/alsa_no_ucm.png)
...
[Sample rate conversion](https://en.wikipedia.org/wiki/Sample_rate_conversion) usually operates at fixed input and output rates. When a client creates a stream, it may enable _variable rate_ mode. In this case, input or output rate may be updated on the fly by explicit client request.
...
A way how the sink and sink input volumes are combined is determined by the _flat volumes_ mode (inspired by Windows Vista):
...
Software volumes use the cubic scale. Hardware volumes generally use an unspecified scale. However, volumes of hardware sources and sinks that have the _decibel volume_ flag and volumes of all sink inputs may be converted to and from the decibel scale using dedicated API functions.

Finally, virtual source and sinks that are attached to a master source or sink usually use _volume sharing_ mode. When it is enabled, the source or sink always uses the same volume as its master.
...
![](/articles/pulseaudio-under-the-hood/diagrams/volumes.png)
...
![](/articles/pulseaudio-under-the-hood/diagrams/aec.png)
...
[![](/articles/pulseaudio-under-the-hood/diagrams/dataflow.png)](/articles/pulseaudio-under-the-hood/diagrams/dataflow.png)
...
![](/articles/pulseaudio-under-the-hood/diagrams/buffers.png)
...
![](/articles/pulseaudio-under-the-hood/diagrams/rewind_buffers.png)
...
![](/articles/pulseaudio-under-the-hood/diagrams/rewind_flow.png)
...
[![](/articles/pulseaudio-under-the-hood/diagrams/sequence.png)](/articles/pulseaudio-under-the-hood/diagrams/sequence.png)
...
For every stream, PulseAudio server maintains a constant latency, depending on _adjust latency_ mode that may be enabled per-stream by an application:
...
Timing info may be retrieved manually by application or automatically when the _auto timing update_ flag is set for a stream.
...
* unless the _not monotonic_ flag is set for the stream, the client ensures that the stream time never steps back;

* if the _interpolate timing_ flag is set for the stream, the client interpolates and smooths the stream time between timing info updates.
...
Non-ALSA backends generally don’t support adjusting device buffer size. An application can determine if a source or sink supports it by checking the _dynamic latency_ flag of the device.
...
To workaround problems with such backends, the user can manually set the _latency offset_ for a device port, which is zero by default. When a source or sink is connected to a device port, the latency offset of the device port is added to the latency (device buffer size) reported by the source or sink.

### ALSA challenges

When a program uses ALSA, the program writes or reads samples from the ALSA ring buffer, and the sound card reads or writes samples from that buffer at a timer tick. The user can configure the _buffer size_ (number of samples in whole ring buffer) and the _period size_ (a.k.a. _fragment size_, the number of samples written or read per one timer tick).
...
* every device may have the “device.intended\_roles” property provided by the device backend
...
* When server backend creates a device, it may specify a list of roles intended for the device via the “device.intended\_roles” property. This property may be also autodetected from the device form factor.
...
Sources and sinks may have “device.form\_factor” property. Form factor is a short string describing device type, e.g. “handset”, “headset”, “speaker”, or “microphone”.
...
* **“filter.suppress”**
    ...
    The name of the filter _not_ to load. The server doesn’t load the filter, even if it’s specified in “filter.want” or “filter.apply”. Useful when “filter.want” or “filter.apply” property is set automatically, but application wants to disable it.
...
* **module-filter-heuristics**
    ...
    Currently, the only filter that may be ignored is “echo-cancel”. It is ignored if the “device.intended\_roles” property of the stream device contains the “phone” role.
...
* **module-filter-apply**
  ...
  * Checks if the user did specify additional module paraterer via the “filter.apply.<filter\_name>.parameters” property.

  * Loads the “module-<filter\_name>” module. The “master” or the “source\_master” and “sink\_master” arguments are set to the name of the source or sink to which the stream or paired streams are currently connected. If the user did specify additional parameters, they are also passed to the module.
...
* **automatic profile switch**
    ...
    When a new stream is created and its “media.role” property is set to “phone” or unset (depending on the “auto\_switch” module parameter), switch the card profile to HSP, to make the bluetooth source available when the regular stream routing logic is executed. When all such source outputs disappear, switch the card profile to A2DP.
...
![](/articles/pulseaudio-under-the-hood/diagrams/alsa_compat.png)
...
![](/articles/pulseaudio-under-the-hood/diagrams/ossp.png)
...
* **exports and imports**
    ...
    When either the exporting process _revokes_ the block or the importing process _releases_ the block, it is returned to the memory pool.
...
This tool uses the C API to communicate with the server. PulseAudio automatically saves most of these setting to the restoration database, so they are persistent.
 ...
 [![](/articles/pulseaudio-under-the-hood/screenshots/pavucontrol_playback_ann.png)](/articles/pulseaudio-under-the-hood/screenshots/pavucontrol_playback_ann.png)[![](/articles/pulseaudio-under-the-hood/screenshots/pavucontrol_recording_ann.png) ](/articles/pulseaudio-under-the-hood/screenshots/pavucontrol_recording_ann.png)[![](/articles/pulseaudio-under-the-hood/screenshots/pavucontrol_output_devices_ann.png) ](/articles/pulseaudio-under-the-hood/screenshots/pavucontrol_output_devices_ann.png)[![](/articles/pulseaudio-under-the-hood/screenshots/pavucontrol_input_devices_ann.png) ](/articles/pulseaudio-under-the-hood/screenshots/pavucontrol_input_devices_ann.png)[![](/articles/pulseaudio-under-the-hood/screenshots/pavucontrol_configuration_ann.png)](/articles/pulseaudio-under-the-hood/screenshots/pavucontrol_configuration_ann.png)
...
This tool just writes module names and arguments to the GNOME registry (GConf) and PulseAudio automatically loads the modules. These settings are persistent.
 ...
 [![](/articles/pulseaudio-under-the-hood/screenshots/paprefs_network_access_ann.png)](/articles/pulseaudio-under-the-hood/screenshots/paprefs_network_access_ann.png)[![](/articles/pulseaudio-under-the-hood/screenshots/paprefs_network_server_ann.png) ](/articles/pulseaudio-under-the-hood/screenshots/paprefs_network_server_ann.png)[![](/articles/pulseaudio-under-the-hood/screenshots/paprefs_multicast_rtp_ann.png) ](/articles/pulseaudio-under-the-hood/screenshots/paprefs_multicast_rtp_ann.png)[![](/articles/pulseaudio-under-the-hood/screenshots/paprefs_simultaneous_output_ann.png)](/articles/pulseaudio-under-the-hood/screenshots/paprefs_simultaneous_output_ann.png)
...
[![](/articles/pulseaudio-under-the-hood/screenshots/qpaeq.png)](/articles/pulseaudio-under-the-hood/screenshots/qpaeq.png)
...
* **Connect a recording application to a source**
    ...
    ![](/articles/pulseaudio-under-the-hood/examples/pnr_app_to_source.png)
...
* **Connect a playback application to a sink**
    ...
    ![](/articles/pulseaudio-under-the-hood/examples/pnr_app_to_sink.png)
...
* **Connect a playback application to a filter sink**
    ...
    ![](/articles/pulseaudio-under-the-hood/examples/pnr_app_to_filter.png)
...
* **Connect a sink to a file**
    ...
    ![](/articles/pulseaudio-under-the-hood/examples/capture_sink_to_file.png)
...
* **Connect a playback application to a file**
    ...
    ![](/articles/pulseaudio-under-the-hood/examples/capture_app_to_file.png)
...
* **Connect a playback application to a recording application**
    ...
    ![](/articles/pulseaudio-under-the-hood/examples/capture_app_to_app.png)
...
* **Connect a local playback application to a remote sink**
    ...
    ![](/articles/pulseaudio-under-the-hood/examples/native_app_to_sink.png)
...
* **Connect a local playback application to a remote recording application**
    ...
    ![](/articles/pulseaudio-under-the-hood/examples/native_app_to_app.png)
...
* **Connect a local sink to a remote sink**
    ...
    ![](/articles/pulseaudio-under-the-hood/examples/native_sink_to_sink.png)
...
* **Connect a local playback application to a remote sink**
    ...
    ![](/articles/pulseaudio-under-the-hood/examples/rtp_app_to_sink.png)
...
* **Connect a local playback application to a remote recording application**
    ...
    ![](/articles/pulseaudio-under-the-hood/examples/rtp_app_to_app.png)
...
* **Connect a local sink to a remote sink**
    ...
    ![](/articles/pulseaudio-under-the-hood/examples/rtp_sink_to_sink.png)
...
We _won’t_ discuss several kinds of issues:
...
$('h2, h3, h4').each(function() { var id = $(this).attr('id'); $(this).prepend( '<a class="anchor" href="#' + id +'"><i class="fa fa-paragraph"></i></a>' ); })
...
Enable JavaScript to view the [comments powered by Disqus.](https://disqus.com/?ref_noscript)
